...
function EditObjective({ id }) {
  ...
  useEffect(() => {
    const objetivoString = localStorage.getItem(id);
    if (objetivoString) {
      const objetivo = JSON.parse(objetivoString);
      setNombre(objetivo.nombre);
      setHorasObjetivo(objetivo.horasObjetivo);
      setHorasPorDia(objetivo.horasPorDia);
      setFechaInicio(objetivo.fechaInicio);
      setDiasNoDisponibles(objetivo.diasNoDisponibles);
    }
  }, [id]);
